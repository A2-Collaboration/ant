...
using WrapTree = triplePi0::PionProdTree;
...
public:
        ...
        nchannels(ExpConfig::Setup::GetDetector<TaggerDetector_t>()->GetNChannels())
    {
        ...
        if(!input.GetObject(triplePi0::SeenTree::treeAccessName(),seen))
            throw Exception("Input TTree for seen mc not found");
        if(!seenTree.Matches(seen))
            throw std::runtime_error("Tree branches don't match");
        seenTree.LinkBranches(seen);

        if(!input.GetObject(triplePi0::RecTree::treeAccessName(),rec))
    ...
    virtual void ProcessEntry(const long long entry) override
    {
        ...
        cuttree::Fill<MCTrue_Splitter<TriplePi0Hist_t>>(signal_hists, {tree, recTree});
    }
...
const BinSettings triplePi0_Plot::taggerBins(47); // TODO: Get from Setup
