...
Setup_2017Plus_NewTagger_Base::Setup_2017Plus_NewTagger_Base(const string& name, OptionsPtr opt) :
    Setup(name, opt),
    ...
    TAPSVeto(make_shared<detector::TAPSVeto_2014>(cherenkovInstalled, pizzaInstalled))
{
    ...
    AddDetector(Trigger);
    //AddDetector(Tagger);  // add Tagger information in derived classes
    AddDetector(CB);
    AddDetector(PID);
    AddDetector(TAPS);
    ...
    AddCalibration<calibration::CB_Energy>(CB, calibrationDataManager, convert_GeSiCa_SADC,
                                           ...
                                           std::vector<double>{thresholds ? 1.9 : 0.0},
    ...
    AddCalibration<calibration::PID_Energy>(PID, calibrationDataManager, convert_MultiHit16bit,
                                            ...
                                            std::vector<double>{0.35},                     // default MC MeV threshold
                                            std::vector<double>{1.0}      // default relative gain
                                            );
    ...
    AddCalibration<calibration::TAPS_Energy>(TAPS, calibrationDataManager, convert_MultiHit16bit,
                                             ...
                                             std::vector<double>{thresholds ? 2.65 : 0.0}, // default MC MeV threshold
                                             std::vector<double>{1.0}  // default relative gain
                                             );
...
double Setup_2017Plus_NewTagger_Base::GetElectronBeamEnergy() const {
    return 883.0;
}

void Setup_2017Plus_NewTagger_Base::BuildMappings(std::vector<ant::UnpackerAcquConfig::hit_mapping_t>& hit_mappings, std::vector<ant::UnpackerAcquConfig::scaler_mapping_t>& scaler_mappings) const
{
...
Setup_traits::candidatebuilder_config_t Setup_2017Plus_NewTagger_Base::GetCandidateBuilderConfig() const
{
...
Setup_traits::triggersimu_config_t Setup_2017Plus_NewTagger_Base::GetTriggerSimuConfig() const
{
...
Setup_traits::target_properties_t Setup_2017Plus_NewTagger_Base::GetTargetProperties() const
{
...
ant::UnpackerA2GeantConfig::promptrandom_config_t Setup_2017Plus_NewTagger_Base::GetPromptRandomConfig() const {
